...
def admin_diag():
    import os, stat as stat_mod, sqlite3 as _sqlite3
    db_path_str = str(db.DB_PATH)
    file_exists = os.path.exists(db_path_str)
    file_size = os.path.getsize(db_path_str) if file_exists else None

    try:
        data_listdir = os.listdir("/data")
    except Exception as e:
        data_listdir = str(e)

    try:
        data_stat = os.stat("/data")
        data_stat_info = {"st_dev": data_stat.st_dev, "st_ino": data_stat.st_ino, "st_mode": data_stat.st_mode}
    except Exception as e:
        data_stat_info = str(e)

    db_counts = {}
    sqlite_sequence = []
    if file_exists:
        try:
            c = _sqlite3.connect(db_path_str)
            db_counts["play_results"] = c.execute("SELECT COUNT(*) FROM play_results").fetchone()[0]
            db_counts["tracked_plays"] = c.execute("SELECT COUNT(*) FROM tracked_plays").fetchone()[0]
            db_counts["max_tracked_id"] = c.execute("SELECT MAX(id) FROM tracked_plays").fetchone()[0]
            db_counts["max_result_id"] = c.execute("SELECT MAX(id) FROM play_results").fetchone()[0]
            sqlite_sequence = c.execute("SELECT * FROM sqlite_sequence").fetchall()
            c.close()
        except Exception as e:
